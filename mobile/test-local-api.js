...
async function testHealth() {
  try {
    ...
    const response = await axios.get(`${LOCAL_BASE_URL}/health`, {
      timeout: 10000,
      headers: {
...
async function testAuthCheck() {
  try {
    const response = await axios.post(`${LOCAL_API_URL}/auth/check`, { 
...
async function testSignIn() {
  try {
    const response = await axios.post(`${LOCAL_API_URL}/auth/signin`, { 
      emailOrPhone: 'janesmith@example.com',
