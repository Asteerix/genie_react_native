...
import { markStoryAsViewed } from '../api/stories';
import { Story, StoryMedia } from '../api/contacts';
...
const StoryViewerScreen = () => {
  ...
  const { storyId, friendId, friendName, friendAvatar, stories } = route.params;
  ...
  useEffect(() => {
    if (currentStory && !currentStory.viewed) {
      // Appeler l'API pour marquer la story comme vue
      const markAsViewed = async () => {
        try {
          const response = await markStoryAsViewed(currentStory.id);
          
          if (response.error) {
            console.error('Erreur API lors du marquage de la story comme vue:', response.error);
            // Mettre quand même à jour l'état local pour la démo
          ...
          console.error('Exception lors du marquage de la story comme vue:', error);
          // Mettre quand même à jour l'état local pour la démo
          currentStory.viewed = true;
        }
      };
        ...
        <View style={styles.userInfo}>
          ...
          <View>
            ...
            <Text style={styles.timestamp}>
              {new Date(
                typeof currentMedia?.timestamp === 'number' 
                  ? currentMedia.timestamp 
                  : currentMedia?.timestamp instanceof Date 
                  ? currentMedia.timestamp.getTime() 
                  : Date.now()
              ).toLocaleTimeString([], {
                hour: '2-digit',
                minute: '2-digit'
