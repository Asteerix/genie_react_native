...
if [ -n "$ACCESS_TOKEN" ]; then
  ...
  test_endpoint "Testing request password reset" "curl -s --max-time $TIMEOUT -X POST -H \"Content-Type: application/json\" -d '{\"emailOrPhone\":\"janesmith@example.com\"}' http://localhost:8080/api/auth/reset"
  
  print_header "6. TOKEN REFRESH"
  test_endpoint "Testing refresh token" "curl -s --max-time $TIMEOUT -X POST -H \"Content-Type: application/json\" -d '{\"refreshToken\":\"$REFRESH_TOKEN\"}' http://localhost:8080/api/auth/refresh"
  
  print_header "7. SIGNOUT"
  test_endpoint "Testing signout with refresh token" "curl -s --max-time $TIMEOUT -X POST -H \"Authorization: Bearer $ACCESS_TOKEN\" -H \"Content-Type: application/json\" -d '{\"refreshToken\":\"$REFRESH_TOKEN\"}' http://localhost:8080/api/auth/signout"
else
